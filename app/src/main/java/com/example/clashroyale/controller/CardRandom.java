...
public class CardRandom {
    ...
    private int[][] cardOrganize = new int[8][2];
    ...
    public void reOrganize() {
       ...
       // archor, giant, fireball, iceWizard, knight, prince, wizard, zap
        ...
        cardOrganize = cardArray;
    }

    public void setStartImageResources(ImageView[] img) {
        for(int i=0; i<5; i++) {
            int id = GlobalConfig.generateIntId(6);
            img[i].setImageResource(cardOrganize[i][0]);
            img[i].setId(cardOrganize[i][1]);
        }
        imgArray = img;
    }
    ...
    public void nextCard(ImageButton selectedButton) {
        ...
        // 把出的牌換成next的卡牌
        selectedButton.setImageResource(cardOrganize[nextInt][0]);
        selectedButton.setId(cardOrganize[nextInt][1]);

        // 把next的卡牌換成再下一張
        int len = imgArray.length;
        int cardIndex = nextInt + 1;
        if (cardIndex > 7) cardIndex = 0;
        imgArray[len-1].setImageResource(cardOrganize[cardIndex][0]);
        imgArray[len-1].setId(cardOrganize[cardIndex][1]);
        nextInt++;
    }
}
